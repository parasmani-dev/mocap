...
for f_idx, frame in enumerate(frames):
    ...
    if l_sh and l_elb:
        dir_arm = norm_v(l_elb - l_sh)
        q_l_arm = quat_between(Vector((1, 0, 0)), dir_arm)
    if pb_l_arm:
    ...
    if l_elb and l_w:
        dir_fore = norm_v(l_w - l_elb)
        q_l_fore_w = quat_between(Vector((1, 0, 0)), dir_fore)
    ...
    # 3. Direct Anatomical Finger Tracking
    def solve_hand_fingers(hand_lms, is_left=False):
        ...
        for fname, base, pbs in finger_defs:
            ...
            if fname == 'Thumb':
                f1 = math.acos(max(-1.0, min(1.0, v_fwd.dot(v01)))) * 0.5
                f2 = math.acos(max(-1.0, min(1.0, v01.dot(v12))))
                f3 = math.acos(max(-1.0, min(1.0, v12.dot(v23))))
            else:
                f1 = math.acos(max(-1.0, min(1.0, v_fwd.dot(v01)))) * 0.8
                f2 = math.acos(max(-1.0, min(1.0, v01.dot(v12))))
                f3 = math.acos(max(-1.0, min(1.0, v12.dot(v23))))
...
def main():
    ...
    with open(solver_script, "w", encoding="utf-8") as f:
        f.write(BLENDER_DIRECT_SOLVER)
        
    json_files = glob.glob(os.path.join(OUTPUT_LANDMARKS_DIR, "*_raw.json"))
    all_json = sorted(list(set(json_files + glob.glob(os.path.join(OUTPUT_LANDMARKS_DIR, "*.json")))))
    
    print(f"Found {len(all_json)} landmark JSON files to re-bake with accurate biological finger movements...")
    
    for i, jpath in enumerate(all_json, 1):
        bname = os.path.basename(jpath)
        clean_word = bname.replace("_raw.json", "").replace(".json", "")
        out_glb = os.path.join(OUTPUT_GLB_DIR, f"avatar_{clean_word}.glb")
        
        print(f"[{i}/{len(all_json)}] Baking {clean_word} -> {os.path.basename(out_glb)}...")
        cmd = [
            BLENDER_EXE,
            "--background",
            ...
            "--landmarks", jpath,
            "--output", out_glb,
            "--anim_name", f"sign_{clean_word}"
        ]
        res = subprocess.run(cmd, capture_output=True, text=True)
        if res.returncode == 0 and os.path.exists(out_glb):
            print(f"  [OK] {clean_word} baked successfully ({os.path.getsize(out_glb)/(1024*1024):.1f} MB)")
        else:
            print(f"  [FAIL] Failed for {clean_word}: {res.stderr[-300:] if res.stderr else res.stdout[-300:]}")
